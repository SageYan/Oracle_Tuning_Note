...
查看挖掘结果
select scn,sql_redo from v$logmnr_contents where seg_name='E01' and seg_owner='SCOTT' and lower(sql_redo) like 'drop%';
SCN             SQL_REDO
1032938         drop table scott.e01 purge;

3.在备用节点还原数据文件，拷贝参数文件和备份的数据文件到生产节点
...
6.拷贝需要的归档日志
scp /home/oracle/arc_db01_dest1/1{27..35}924710753.dbf oracle@172.25.5.11:/home/oracle/arc_db01_dest1/
...
8. 使用数据泵导出被误操作的表，导入到生产库
--ps:若是误删除表空间，不完全恢复的方法类似，表空间通过exp传输表空间到原始端
```
